...
#define GPS_USART_PORT                 USART1 // Port connected to the GPS
...
typedef struct {
	...
	uint8_t     datetime_valid;      // Have valid date and time
	uint8_t     valid;               // GPS status: TRUE if data valid
} GPS_Data_TypeDef;
...
typedef struct {
	...
	uint8_t     used;                // TRUE if satellite used in location fix
} GPS_Satellite_TypeDef;

typedef struct {
	uint8_t     PMTK_BOOT;           // TRUE when "$PMTK011,MTKGPS*08" sentence parsed
...
extern uint8_t GPS_new_data;                          // TRUE if received new GPS packet
extern uint8_t GPS_parsed;                            // TRUE if GPS packets was parsed
