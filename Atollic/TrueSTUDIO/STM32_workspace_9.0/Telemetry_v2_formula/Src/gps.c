...
uint8_t GPS_new_data;                         // TRUE if received new GPS packet
uint8_t GPS_parsed;                            // TRUE if GPS packets was parsed
...
void GPS_FindSentence(NMEASentence_TypeDef *Sentence, uint8_t *buf,
		uint16_t start, uint16_t buf_size) {
	uint16_t pos = start;
	uint32_t hdr;
	...
	do {
		...
		if (1) {
			...
			Sentence->end = pos + 1;
			if (hdr == 0x00474c4c) {
				Sentence->type = NMEA_GLL;
				return;
			}
			if (hdr == 0x00524d43) {
				Sentence->type = NMEA_RMC;
				return;
			}
			if (hdr == 0x00565447) {
				Sentence->type = NMEA_VTG;
				return;
			}
			if (hdr == 0x00474741) {
				Sentence->type = NMEA_GGA;
				return;
			}
			if (hdr == 0x00475341) {
				Sentence->type = NMEA_GSA;
				return;
			}
			if (hdr == 0x00475356) {
				Sentence->type = NMEA_GSV;
				return;
			}
			if (hdr == 0x005a4441) {
				Sentence->type = NMEA_ZDA;
				return;
			}
...
// return: number of parsed bytes
uint16_t GPS_ParseCoordinate(uint8_t *buf, uint8_t len, uint32_t *value,
		uint8_t *char_value) {
	uint16_t pos = 0;
	...
	if (buf[pos] != ',') {
		...
		*value = atos_len(&buf[pos], len) * 1000000; // degrees
		...
		if (f_len > 0) {
			...
			*value += (uint32_t) ((coord_minutes / (pwr10(f_len) * 60.0))
					* 1000000);
		} else {
...
void GPS_ParseSentence(uint8_t *buf, NMEASentence_TypeDef *Sentence) {
	...
	case NMEA_RMC:
		...
		// Time of fix
		if (buf[pos] != ',')
			pos += GPS_ParseTime(&buf[pos], &GPSData.fix_time);
		else
			pos++;
		...
		// Latitude
		pos += GPS_ParseCoordinate(&buf[pos], 2, &GPSData.latitude,
				&GPSData.latitude_char);

		// Longitude
		pos += GPS_ParseCoordinate(&buf[pos], 3, &GPSData.longitude,
				&GPSData.longitude_char);

		// Horizontal speed (in knots)
		pos += GPS_ParseFloat(&buf[pos], &GPSData.speed_k);

		// Convert speed in knots to speed in km/h
		if (GPSData.speed == 0 && GPSData.speed_k != 0)
			GPSData.speed = (GPSData.speed_k * 1852) / 1000;
	...
	case NMEA_ZDA:
		...
		if (GPSData.date % 10000 > 2013)
			GPSData.datetime_valid = 1;
...
uint8_t * append_GPS_Data(uint8_t *msg, uint16_t *msg_len) {

	// TODO: podmienka: ak su data validne a mam fix > 2, tak pokracujem, inak return
	if(GPSData.fix < 3)
	{
		return msg;
	}

	uint8_t dataSize = 22 + 2;
	uint16_t dataID = 5;	// 5 for gps data
	msg = (uint8_t *) realloc(msg, *msg_len + dataSize * sizeof(uint8_t));

	int index = *msg_len;
	msg[index++] = dataID & 0xff;
	msg[index++] = (dataID >> 8);
	msg[index++] = (GPSData.latitude >> 24) & 0xff;
	msg[index++] = (GPSData.latitude >> 16) & 0xff;
	msg[index++] = (GPSData.latitude >> 8) & 0xff;
	msg[index++] = (GPSData.latitude >> 0) & 0xff;

	msg[index++] = GPSData.latitude_char;

	msg[index++] = (GPSData.longitude >> 24) & 0xff;
	msg[index++] = (GPSData.longitude >> 16) & 0xff;
	msg[index++] = (GPSData.longitude >> 8) & 0xff;
	msg[index++] = (GPSData.longitude >> 0) & 0xff;

	msg[index++] =GPSData.longitude_char;

	msg[index++] = (GPSData.speed >> 24) & 0xff;
	msg[index++] = (GPSData.speed >> 16) & 0xff;
	msg[index++] = (GPSData.speed >> 8) & 0xff;
	msg[index++] = (GPSData.speed >> 0) & 0xff;

	msg[index++] = (GPSData.course >> 24) & 0xff;
	msg[index++] = (GPSData.course >> 16) & 0xff;
	msg[index++] = (GPSData.course >> 8) & 0xff;
	msg[index++] = (GPSData.course >> 0) & 0xff;

	msg[index++] = (GPSData.altitude >> 24) & 0xff;
	msg[index++] = (GPSData.altitude >> 16) & 0xff;
	msg[index++] = (GPSData.altitude >> 8) & 0xff;
	msg[index++] = (GPSData.altitude >> 0) & 0xff;
