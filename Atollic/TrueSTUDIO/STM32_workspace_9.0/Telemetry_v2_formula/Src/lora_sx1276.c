...
// Reads single register
 uint8_t read_register(lora_sx1276 *lora, uint8_t address)
{
  uint8_t value = 0;
...
uint8_t lora_send_packet_blocking(lora_sx1276 *lora, uint8_t *data, uint8_t data_len, uint32_t timeout)
{
  ...
  if (res == LORA_OK) {
    ...
    while (elapsed < timeout) {
      ...
      HAL_Delay(1);
      elapsed++; // 400 delay on read + 400 delay in write
    }
  }
...
uint8_t lora_init_explicit(lora_sx1276 *lora, SPI_HandleTypeDef *spi, GPIO_TypeDef *nss_port,
